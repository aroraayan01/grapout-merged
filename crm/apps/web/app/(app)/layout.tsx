...
import { api, BASE_PATH, hasStashedAdmin, restoreAdminSession } from '@/lib/api';
  ...
  return (
    <div className="flex min-h-screen">
      ...
      >
        ...
        {/*
         * The way back to Trade.
         *
         * Only when this is being served inside grapout.com — BASE_PATH is set
         * exactly then. On its own domain there is no /trade to go to, and a
         * button that leads nowhere is worse than no button.
         *
         * A plain anchor, not a Link: Trade is a different program next door,
         * and Next's router would try to handle the route itself.
         */}
        {BASE_PATH && (
          <a
            href="/trade"
            className="mx-3 mb-2 flex items-center gap-3 rounded-xl bg-white/10 px-3 py-2.5 ring-1 ring-white/10 transition hover:bg-white/15"
          >
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-white/90 text-sm font-black text-brand-700">
              G
            </span>
            <span className="min-w-0 flex-1 leading-tight">
              <span className="block truncate text-sm font-semibold text-white">GrapMe</span>
              <span className="block truncate text-[11px] text-indigo-300">Switch to GrapOut Trade</span>
            </span>
            <svg
