...
export default function EmailLogPage() {
  ...
  const [logMsg, setLogMsg] = useState<{ id: string; name: string } | null>(null);
  ...
  useEffect(() => {
    ...
    api.get<{ items: Row[]; total: number }>(`/email/log?${params}`)
  ...
  return (
    <div>
      <PageHeader title="Email Log" subtitle="Every email sent across clients — who opened, clicked, replied, bounced or forwarded. Read-only." />
      ...
      {!loaded ? (
        ...
        <>
              ...
              <tbody>
                {items.map((m) => (
                  <tr key={m.id} className="border-t border-slate-100 hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3">
                      {m.client
                        ? <Link href={`/linkedin/${m.client.id}`} className="text-slate-700 hover:underline">{m.client.name}</Link>
                    ...
                    <td className="px-4 py-3 text-right">
                      <button className="text-xs font-medium text-slate-600 hover:underline" onClick={() => setLogMsg({ id: m.id, name: m.contact?.name ?? m.contact?.email ?? 'Recipient' })} title="See the full activity timeline">Log</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          {logMsg && <EmailLogModal messageId={logMsg.id} name={logMsg.name} onClose={() => setLogMsg(null)} />}
        </>
      )}
    </div>
  );
