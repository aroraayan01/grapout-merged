...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    /**
     * Calls or meetings switched off for this account — on the account
     * itself, or on the company page it represents, which switches off
     * everybody on that team.
     */
    public function featureBlocked(string $feature): bool
    {
        $column = $feature . '_disabled';
        if ((bool) $this->{$column}) {
            return true;
        }
        $page = $this->relationLoaded('businessPage') ? $this->businessPage : $this->businessPage()->first();

        // The team's lock, or the owner's own switch.
        return (bool) ($page?->{$column} ?? false) || ($page !== null && ! ($page->{'accept_' . $feature} ?? true));
    }

    /**
     * The company this person represents — through the team table, so a
     * page has many people and a person has one page. Only an active
     * membership counts; an invitation or a request is not a seat.
     *
     * A staff member acting for a page (the admin's page picker sends
     * X-Acting-Page) wears that page instead, as its owner would.
     */
    /** Set per request by ResolveActingPage: the page a staff member wears, and who they are. */
    public static ?array $acting = null;

    public function businessPage(): \Illuminate\Database\Eloquent\Relations\Relation
    {
        // Eager loading builds the relation on a blank instance (id null), so
        // the decision rests on the request, not on $this.
        if (self::$acting && ($this->id === null || $this->id === self::$acting['user_id'])) {
            return $this->hasOne(\App\Models\Business\Page::class, 'id', 'acting_page_id');
        }

        return $this->hasOneThrough(\App\Models\Business\Page::class, \App\Models\Business\CompanyMember::class, 'user_id', 'id', 'id', 'page_id')
            ->where('company_members.status', 'active');
    }

    /** The page named in the X-Acting-Page header of this request, if a staff member sent it. */
    public static function actingPageId(): ?int
    {
        return self::$acting['page_id'] ?? null;
    }

    /** Staff only: is this person making the request for a chosen page? */
    public function actsForPage(): bool
    {
        return self::$acting !== null && self::$acting['user_id'] === $this->id;
    }

    /** Read by the acting relation above; never stored. */
    public function getActingPageIdAttribute(): ?int
    {
        return $this->actsForPage() ? self::actingPageId() : null;
    }

    public function companyMembership(): \Illuminate\Database\Eloquent\Relations\HasOne
    {
        return $this->hasOne(\App\Models\Business\CompanyMember::class)->where('status', 'active');
    }

    /** Pages this person follows. */
    public function followedPages(): BelongsToMany
    {
        return $this->belongsToMany(\App\Models\Business\Page::class, 'business_follows', 'user_id', 'page_id')->withTimestamps();
    }
    ...
    public function presenceAudience(int $limit = 200): array
    {
        ...
        $connected = Connection::query()
            ...
            // Plain ids from here on: an Eloquent collection will not merge with them.
            ->toBase()
    ...
    /**
     * The top of the platform. Admin is that role; super_admin is the older
     * name kept for the seeded account, and means the same thing everywhere.
     */
