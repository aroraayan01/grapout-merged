...
class UserController extends Controller
{
    ...
    public function store(Request $request, AppIdService $appIds): JsonResponse
    {
        ...
        // Only an admin may create admins (or other super admins).
        if (in_array($data['role'], ['admin', 'super_admin']) && ! $request->user()->isSuperAdmin()) {
            abort(403, 'Only an admin can create admin accounts.');
        }
    ...
    /** Calls and meetings on or off for one account. */
    public function features(Request $request, User $user): JsonResponse
    {
        $this->guardTargetEditable($request, $user);
        $data = $request->validate([
            'calls' => ['sometimes', 'boolean'],
            'meetings' => ['sometimes', 'boolean'],
            // Hot Leads for this account with no daily limits, whatever its plan.
            'grap_leads' => ['sometimes', 'boolean'],
            'connection_daily_limit' => ['sometimes', 'nullable', 'integer', 'min:0', 'max:1000'],
        ]);
        $user->forceFill(array_filter([
            'calls_disabled' => array_key_exists('calls', $data) ? ! $data['calls'] : null,
            'meetings_disabled' => array_key_exists('meetings', $data) ? ! $data['meetings'] : null,
            'grap_leads_granted' => array_key_exists('grap_leads', $data) ? $data['grap_leads'] : null,
        ], fn ($v) => $v !== null));
        if (array_key_exists('connection_daily_limit', $data)) {
            $user->forceFill(['connection_daily_limit' => $data['connection_daily_limit']]); // null = the platform's number
        }
        $user->save();
        \App\Models\AuditLog::record($request->user(), 'user.features', $user, $data);

        return response()->json(['message' => 'Saved.', 'data' => new UserResource($user->fresh()->load(['appId', 'roles', 'profile']))]);
    }
    ...
    public function syncRoles(Request $request, User $user): JsonResponse
    {
        ...
        if ($touchesAdmin && ! $request->user()->isSuperAdmin()) {
            abort(403, 'Only an admin can grant or change admin roles.');
        }
    ...
    {
        abort_unless($request->user()->isSuperAdmin(), 403, 'Only an admin can regenerate App IDs.');
    ...
    {
        abort_unless($request->user()->isSuperAdmin(), 403, 'Only an admin can change settings.');
        ...
        $data = $request->validate([
            ...
            'translate_provider' => ['sometimes', 'in:none,google,deepl,libre,claude'],
            'translate_key' => ['sometimes', 'nullable', 'string', 'max:300'],
            'translate_url' => ['sometimes', 'nullable', 'string', 'max:300'],
            'trade_currencies' => ['sometimes', 'string', 'max:800', 'regex:/^[A-Z]{3}(,[A-Z]{3})*$/'],
            'connection_daily_limit' => ['sometimes', 'integer', 'min:0', 'max:1000'],
        ]);
        if (array_key_exists('translate_key', $data)) {
            if ($data['translate_key'] === null) {
                $data['translate_key'] = '';
            } elseif (trim($data['translate_key']) === '') {
                unset($data['translate_key']);
            }
        }
        if (array_key_exists('translate_url', $data)) {
            $data['translate_url'] = trim((string) $data['translate_url']);
        }
        ...
        foreach (['voice_ai_enabled'] as $flag) {
