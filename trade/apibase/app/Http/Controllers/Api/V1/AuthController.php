...
class AuthController extends Controller
{
    public function register(Request $request, AppIdService $appIds): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            // Optional since the trade sign-up asks for a company, not a handle:
            // one is derived from the company or the name when none is sent.
            'username' => ['nullable', 'string', 'min:4', 'max:20', 'regex:/^[a-zA-Z0-9]+$/'],
            // The company this person represents, and what it is. Its page opens with the account.
            'company_name' => ['nullable', 'string', 'min:2', 'max:160'],
            'kind' => ['nullable', Rule::in(\App\Models\Business\Page::KINDS)],
            // Or the company already here that this person works for: no new
            // page, a request to its owner instead, with what they do there.
            'join_page' => ['nullable', 'string', 'exists:business_pages,slug'],
            'function' => ['nullable', Rule::in(\App\Models\Business\CompanyMember::FUNCTIONS)],
        ...
        $username = ! empty($data['username'])
            ...
            : $this->freeUsername(trim((string) ($data['company_name'] ?? '')) ?: $data['name']);
        ...
        $user = DB::transaction(function () use ($data, $appIds, $username) {
            ...
            $user->profile()->create([
                ...
                // Everybody here trades: an account is a business unless it says otherwise.
            ...
            /*
             * The company page, open from the first minute.
             *
             * Products, trade lines and the rest come later from My Company;
             * what matters is that an enquiry, a match or a follower has
             * somewhere to land the moment this person is in.
             */
            if (! empty($data['join_page'])) {
                $joining = \App\Models\Business\Page::live()->where('slug', $data['join_page'])->first();
                if ($joining) {
                    \App\Models\Business\CompanyMember::create([
                        'page_id' => $joining->id, 'user_id' => $user->id, 'role' => 'representative',
                        'function' => $data['function'] ?? null, 'status' => 'requested',
                    ]);
                }
            } elseif (! empty($data['company_name'])) {
                $page = \App\Models\Business\Page::create([
                    'name' => trim($data['company_name']),
                    'kind' => $data['kind'] ?? 'exporter',
                    'country' => ! empty($data['country']) && strlen($data['country']) === 2 ? strtoupper($data['country']) : null,
                    'user_id' => $user->id,
                    'slug' => \App\Models\Business\Page::slugFor($data['company_name']),
                ]);
                $page->members()->create(['user_id' => $user->id, 'role' => 'owner', 'function' => 'management', 'status' => 'active', 'joined_at' => now()]);
            }
        ...
        if ($inviter) {
            Connection::firstOrCreate(
                ...
                ['status' => 'pending', 'message' => 'Joined GrapOut from your invite link.'],
            );
        }

        // The company's owner and admins hear about the request at once.
        if (! empty($data['join_page'])) {
            $joining = \App\Models\Business\Page::live()->where('slug', $data['join_page'])->first();
            $member = $joining?->members()->where('user_id', $user->id)->first();
            if ($joining && $member) {
                $joining->notifyTeam(new \App\Notifications\SocialNotification(
                    'company_team', "{$user->name} registered and asked to join the team of {$joining->name}.",
                    ['page_slug' => $joining->slug, 'member_id' => $member->id], '/business?tab=team', 'company-request-' . $member->id,
                ), managersOnly: true);
            }
        }
    ...
    public function suggestUsername(Request $request): JsonResponse
    {
        ...
        return response()->json(['data' => ['suggestion' => $this->freeUsername($data['name'] ?? ''), 'available' => true]]);
    }

    /** A handle nobody has, derived from a name: letters and digits only, lowercase, a numeric suffix on collision. */
    private function freeUsername(string $seed): string
    {
        $base = mb_strtolower(preg_replace('/[^a-zA-Z0-9]/', '', $seed));
        if (mb_strlen($base) < 4) {
    ...
    public function verifyEmailOtp(Request $request, \App\Services\MobileOtpService $otps): JsonResponse
    {
        ...
        \App\Models\Business\Enquiry::adoptFor($request->user()->fresh());
    ...
    public function verifyEmail(Request $request, string $id, string $hash): JsonResponse
    {
        ...
        if (! $user->hasVerifiedEmail()) {
            ...
            \App\Models\Business\Enquiry::adoptFor($user->fresh());
        }
