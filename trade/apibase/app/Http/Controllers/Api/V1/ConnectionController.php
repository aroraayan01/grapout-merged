...
class ConnectionController extends Controller
{
    ...
    public function store(Request $request, AppIdService $appIds): JsonResponse
    {
        ...
        // So many a day: the platform's number, or this account's own.
        $limit = $me->connection_daily_limit ?? (int) \App\Models\AppSetting::get('connection_daily_limit');
        $sentToday = Connection::where('requester_id', $me->id)->where('created_at', '>=', now()->startOfDay())->count();
        if ($limit > 0 && $sentToday >= $limit) {
            return response()->json(['message' => "You have sent {$limit} connection requests today, the most for one day. More tomorrow."], 429);
        }
