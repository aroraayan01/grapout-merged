...
// The daily look at what arrived, for everybody with a saved search.
Schedule::command('mypa:trade-alerts')->dailyAt('07:00')->withoutOverlapping();

// Outreach campaigns: the ones whose hour has come, and the ones the daily allowance held back.
Schedule::command('trade:outreach-campaigns')->everyFiveMinutes()->withoutOverlapping();

// Who comes first in a search: re-ranked nightly so yesterday's work counts and month-old work stops counting.
Schedule::command('trade:rank')->dailyAt('02:30')->withoutOverlapping();

// What came back to the outreach mailboxes.
Schedule::command('trade:outreach-inbox')->everyTenMinutes()->withoutOverlapping();
