...
class Phase8BillingTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // The sums below were written against a 99-a-month plan; the price itself is the team's to set.
        \App\Models\Plan::where('slug', 'gold')->update(['monthly_price' => 99, 'annual_price' => 990]);
    ...
    protected function checkout(string $plan = 'gold', string $frequency = 'monthly', ?string $coupon = null): array
    {
    ...
    public function test_percent_coupon_with_cap(): void
    {
        ...
        $this->actingAs($this->user)->postJson('/api/v1/subscription/quote', [
            'plan_slug' => 'gold', 'frequency' => 'monthly', 'coupon' => 'HALF',
        ])->assertOk()
    ...
    public function test_invalid_coupons_rejected(): void
    {
        ...
        Coupon::create([
            ...
            'discount_value' => 1000, 'applicable_plans' => ['diamond'], 'is_active' => true,
        ]);

        $this->actingAs($this->user)->postJson('/api/v1/subscription/quote', [
            'plan_slug' => 'gold', 'frequency' => 'monthly', 'coupon' => 'EXPIRED',
        ])->assertUnprocessable();

        $this->actingAs($this->user)->postJson('/api/v1/subscription/quote', [
            'plan_slug' => 'gold', 'frequency' => 'monthly', 'coupon' => 'FAMONLY',
        ])->assertUnprocessable();
    }
    ...
    public function test_paid_order_activates_subscription_idempotently(): void
    {
        ...
        $this->actingAs($this->user)->getJson('/api/v1/subscription')
            ->assertJsonPath('data.plan.slug', 'gold');
    ...
    public function test_renewal_reminder_sent_at_configured_interval(): void
    {
        ...
        Subscription::create([
            'user_id' => $this->user->id,
            'plan_id' => \App\Models\Plan::where('slug', 'gold')->first()->id,
            'status' => 'active',
    ...
    public function test_partial_refund_and_over_refund_guard(): void
    {
        ...
        // A sub-admin cannot refund; an admin is the top of the platform and can.
        $sub = User::factory()->create();
        $sub->roles()->attach(Role::where('slug', 'subadmin')->first()->id);
