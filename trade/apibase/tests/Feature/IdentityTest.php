...
class IdentityTest extends TestCase
{
    ...
    public function test_admin_can_view_and_resend_otp_and_edit_settings(): void
    {
        ...
        // An admin is the top of the platform and edits settings too; a sub-admin does not.
        $admin = $this->makeApprover('admin');
        $this->actingAs($admin)
            ->putJson('/api/v1/admin/settings', ['username_change_days' => 10])
            ->assertOk();
        $this->assertEquals('10', AppSetting::get('username_change_days'));
        $sub = $this->makeApprover('subadmin');
        $this->app['auth']->forgetGuards();
        $this->actingAs($sub)
            ->putJson('/api/v1/admin/settings', ['username_change_days' => 5])
            ->assertForbidden();
    }
